...
# Упаковываем локально во временный файл (.env берем с собой!)
...
# Распаковка и запуск. Сломанный кэш сессии: файл в data/ создаёт контейнер
# (root), поэтому сначала пробуем rm напрямую, иначе — через docker exec.
