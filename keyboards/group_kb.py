...
class ServerCb(CallbackData, prefix="srv"):
    server_id: int
    action: str  # "start" | "stop" | "backup" | "confirm"
...
# Выбор серверов при привязке чата (/link).
class LinkServerCb(CallbackData, prefix="link"):
    server_id: int   # 0 — системные действия (done/cancel)
    action: str      # "toggle" | "done" | "cancel"


def get_link_kb(servers: list[dict], selected: set[int]) -> InlineKeyboardMarkup:
    """Пикер серверов владельца для привязки чата (/link)."""
        ...
        buttons.append(
            [
                InlineKeyboardButton(
                    text=f"{checked} {name}",
                    callback_data=LinkServerCb(server_id=sid, action="toggle").pack(),
                )
            ]
        )
    buttons.append(
        [
            InlineKeyboardButton(
                text=f"✅ Готово ({len(selected)}/{len(servers)})",
                callback_data=LinkServerCb(server_id=0, action="done").pack(),
            ),
            InlineKeyboardButton(
                text="❌ Отмена",
                callback_data=LinkServerCb(server_id=0, action="cancel").pack(),
            ),
        ]
    )
...
def get_dashboard_kb(servers: list[dict], chat_id: int) -> InlineKeyboardMarkup:
    """Кнопки дашборда: по одной строке на сервер (старт/стоп/бэкап).
    ...
    for s in servers:
        ...
        if is_online:
            ...
            row.append(
                InlineKeyboardButton(
                    text="📦 Бэкап",
                    callback_data=ServerCb(server_id=server_id, action="backup").pack(),
                )
            )
        else:
            row.append(
